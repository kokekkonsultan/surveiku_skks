...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		// Tambahan
		if ($additional_data['is_trial'] == 1) {

			// Cek Panjang Berlangganan
			$this->db->select('panjang_hari');
			$this->db->from('paket');
			$this->db->where('id', $additional_data['id_paket_trial']); // 6 id trial
			$panjang_hari = $this->db->get()->row()->panjang_hari;

			$object = [
				'uuid' 	=> $additional_data['uuid'],
				'id_user' 	=> $id,
				'id_paket' 	=> $additional_data['id_paket_trial'],
				'id_metode_pembayaran' 	=> 3,
				'id_status_berlangganan' => 1,
				'tanggal_mulai' 	=> date('Y-m-d'),
				'tanggal_selesai' 	=> date('Y-m-d', strtotime("+$panjang_hari days", strtotime(date('Y-m-d')))),
				'kode_lisensi' 	=> "XXXX-XXXX-XXXX-XXXX",
			];
			$this->db->insert('berlangganan', $object);
			
		} elseif ($additional_data['is_trial'] == 2) {
			...
			if ($this->input->post('id_paket')) {
				...
				$this->db->select('panjang_hari');
				$this->db->from('paket');
				$this->db->where('id', $additional_data['id_paket']);
				$panjang_hari = $this->db->get()->row()->panjang_hari;

				$object = [
					'uuid' 	=> $additional_data['uuid'],
					'id_user' 	=> $id,
					'id_paket' 	=> $additional_data['id_paket'],
					'id_metode_pembayaran' 	=> $additional_data['id_metode_pembayaran'],
					'id_status_berlangganan' => 1,
					'tanggal_mulai' 	=> $additional_data['tanggal_mulai'],
					'tanggal_selesai' 	=> date('Y-m-d', strtotime("+$panjang_hari days", strtotime($additional_data['tanggal_mulai']))),
	...
	public function update($id, array $data)
	{
		...
		if ($this->input->post('id_paket')) {
			...
			$object = [
				...
				'tanggal_selesai' 	=> date('Y-m-d', strtotime("+$panjang_hari days", strtotime($this->input->post('tanggal_mulai')))),
				'kode_lisensi' 	=> null,
			];
