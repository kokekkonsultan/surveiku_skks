...
class DataPerolehanSurvei_model extends CI_Model
{
    ...
    private function _get_datatables_query($table_identity, $profil_responden)
    {
        ...
        $this->db->select("*, responden_$table_identity.uuid AS uuid_responden, (SELECT first_name FROM users WHERE users.id = surveyor.id_user) AS first_name, (SELECT last_name FROM users WHERE users.id = surveyor.id_user) AS last_name, $query_profil, (SELECT nama_sektor FROM sektor_$table_identity WHERE responden_$table_identity.sektor = sektor_$table_identity.id) AS sektor, (SELECT nama_wilayah FROM wilayah_survei_$table_identity WHERE responden_$table_identity.wilayah_survei = wilayah_survei_$table_identity.id) AS wilayah_survei");
        ...
        if($this->uri->segment(3) == 'data-perolehan-online'){
        ...
        if ($this->input->post('is_surveyor')) {
            $this->db->where('IF(id_surveyor != 0,1,2) =' . $this->input->post('is_surveyor'));
        }
        ...
        $profil_responden_filter = $this->db->query("SELECT * FROM profil_responden_$table_identity WHERE jenis_isian = 1");
