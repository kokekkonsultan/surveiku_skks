...
class RekapHasilKeseluruhanController extends CI_Controller
{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = anchor('rekap-hasil-keseluruhan/rekap-alasan/' . $value->id, 'Detail Alasan <i class="fa fa-arrow-right"></i>', ['class' => 'btn btn-light-primary btn-sm font-weight-bold shadow']);
	...
	public function detail_rekap_alasan()
	{
		...
		$this->data['pertanyaan'] = $this->db->get_where("pertanyaan_unsur_$manage_survey->table_identity", array('id' => $this->uri->segment(3)))->row();
	...
	public function cetak_alasan()
	{
		...
		$klien_induk = $this->db->get_where("pengguna_klien_induk", array('id_user' => $this->session->userdata('user_id')))->row();
		$parent = implode(", ", unserialize($klien_induk->cakupan_induk));
		$manage_survey = $this->db->query("SELECT * FROM manage_survey WHERE id IN ($parent)");
		$last_manage = $manage_survey->last_row();

		$this->data['pertanyaan'] = $this->db->query("SELECT *, (SELECT kode_unsur FROM unsur_$last_manage->table_identity WHERE pertanyaan_unsur_$last_manage->table_identity.id_unsur = unsur_$last_manage->table_identity.id) AS kode_unsur
		FROM pertanyaan_unsur_$last_manage->table_identity");
