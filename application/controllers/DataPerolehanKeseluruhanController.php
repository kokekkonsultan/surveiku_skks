...
class DataPerolehanKeseluruhanController extends CI_Controller
{
	...
	public function ajax_list()
	{
		...
		$data = array();
		foreach ($this->db->query("SELECT *, (SELECT first_name FROM users WHERE users.id = manage_survey.id_user) AS nama_depan, (SELECT last_name FROM users WHERE users.id = manage_survey.id_user) AS nama_belakang FROM manage_survey WHERE id IN ($parent)")->result() as $key => $value) {
