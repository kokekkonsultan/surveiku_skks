...
class Auth extends CI_Controller
{
	...
	public function ajax_list_administrator()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = anchor("pengguna-administrator/edit_administrator/" . $value->user_id, 'Edit', ['class' => 'btn btn-secondary btn-sm font-weight-bold']);
			$row[] = anchor("auth/delete_user/" . $value->user_id, 'Delete', ['class' => 'btn btn-secondary btn-sm font-weight-bold', 'onclick' => "return confirm('Anda yakin ingin menghapus pengguna ?')"]);
			$data[] = $row;
		}
	...
	public function ajax_list_klien()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = $value->last_name;
			$row[] = $value->email;
			$row[] = $value->name; //anchor("auth/edit_group/" . $value->group_id, htmlspecialchars($value->name, ENT_QUOTES, 'UTF-8'));
			$row[] = ($value->active) ? anchor("auth/deactivate/" . $value->user_id, lang('index_active_link')) : anchor("auth/activate/" . $value->user_id, lang('index_inactive_link'));

			$row[] = '<a class="btn btn-light-primary text-white shadow" data-toggle="modal" title="Detail Klien" onclick="showuserdetail(' . $value->user_id . ')" href="#modal_userDetail"><i class="fas fa-key"></i></a>';

			$row[] = anchor("auth/edit_klien/" . $value->user_id, 'Edit', ['class' => 'btn btn-light-primary btn-sm font-weight-bold shadow-lg']);
			$row[] = anchor("auth/delete_klien/" . $value->user_id, 'Delete', ['class' => 'btn btn-light-primary btn-sm font-weight-bold shadow-lg', 'onclick' => "return confirm('Anda yakin ingin menghapus pengguna ?')"]);
  ...
  <tr>
	...
	public function random_string($long_string)
	{
		...
		for ($i = 0; $i < $long_string; $i++) {
			...
			// $string .= $karakter{$pos};
			$string .= $karakter[$pos]; // Penulisan di php 7.4
		}
		return $string;
	}
	...
	public function ajax_list_surveyor()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = anchor("pengguna-surveyor/delete-user/" . $value->user_id, 'Delete', ['class' => 'btn btn-secondary btn-sm font-weight-bold', 'onclick' => "return confirm('Anda yakin ingin menghapus pengguna ?')"]);
